...
- The pull request title is `type(scope): summary` with one of the eleven standard types (`ci / pr-title`). Do not invent types; extra meaning goes in the scope. The description becomes the squash commit body when merged as `CONTRIBUTING.md` step 7 says and, when AI wrote or assisted, ends with `Assisted-by: <agent>:<model>` on its own last line. That is the only line that marks AI: remove a tool's default `Co-Authored-By` for the AI and its generated-with line, in commits too. A close, fix or resolve word before `#N` anywhere in the description closes that issue on merge, so word such a sentence another way.
